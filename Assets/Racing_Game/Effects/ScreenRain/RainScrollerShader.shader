    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                half dropletAlpha = saturate((droplet.a + droplet.b * 0.5h) * _Droplets_Strength);
                half rivuletAlpha = saturate((rivulet.a + rivulet.b * 0.5h) * (_RivuletsStrength * 0.5h));
                half alpha = saturate(max(dropletAlpha, rivuletAlpha));

                half3 tint = _Tint.rgb;
                half3 detail = lerp(droplet.rgb, rivulet.rgb, saturate(rivuletAlpha));
                half3 color = lerp(tint * 0.6h, tint, saturate(detail.r + detail.g + detail.b));
