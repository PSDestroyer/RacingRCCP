...
public class RTC_CarController : RTC_Core {
...
#if BCG_URP || BCG_HDRP
        [HideInInspector] public LensFlareComponentSRP flare;
#else
...
#if BCG_URP || BCG_HDRP
                    if (lights[i].flare == null && lights[i].light.TryGetComponent(out LensFlareComponentSRP flareComponent))
    ...
    public void Lighting(Lights lightSource, float targetIntensity, float smoothness) {
        ...
        if (lightSource.meshRenderer != null && lightSource.meshRenderer.materials.Length > 0 && lightSource.shaderKeyword != "") {
            ...
            lightSource.meshRenderer.materials[lightSource.materialIndex].SetColor(lightSource.shaderKeyword, lightSource.light.intensity * .004f * lightSource.light.color);
            lightSource.meshRenderer.materials[lightSource.materialIndex].SetFloat("_EmissiveExposureWeight", .5f);
...
#else
                if (paints[i] != null && (paints[i].colorString == "" || paints[i].colorString == "_Color"))
        ...
        if (lights != null && lights.Length >= 1) {
            ...
            for (int i = 0; i < lights.Length; i++) {
                ...
                if (lights[i] != null) {
                    ...
                    if (lights[i].intensity < 100f)
                        lights[i].intensity = 600f;

#endif

                    if (lights[i].flareIntensity == 0)
                        lights[i].flareIntensity = .85f;

                    if (lights[i].smoothness == 0)
                        lights[i].smoothness = 20f;

#if BCG_HDRP
                    if (lights[i].shaderKeyword == "" || lights[i].shaderKeyword == "_EmissionColor")
                        lights[i].shaderKeyword = "_EmissiveColor";
#else
    ...
    /// </summary>
    private void LensFlare(Light lightSource, LensFlareComponentSRP flare, float flareIntensity) {
...
#if BCG_HDRP
        flare.intensity = finalFlareBrightness * (lightSource.intensity * .004f);
#else
        flare.intensity = finalFlareBrightness * lightSource.intensity;
#endif
