...
/// </summary>
public static class RTC_AssetUtilities {
    ...
    private static string GetScriptPath() {
        // look for any MonoScript asset named RTC_AssetUtilities
        string[] guids = AssetDatabase.FindAssets("t:MonoScript RTC_AssetUtilities");

        if (guids != null && guids.Length > 0) {
            return AssetDatabase.GUIDToAssetPath(guids[0]);
        }

        Debug.LogError("RTC_AssetUtilities: Failed to locate script via AssetDatabase.FindAssets");
        return string.Empty;
    }
