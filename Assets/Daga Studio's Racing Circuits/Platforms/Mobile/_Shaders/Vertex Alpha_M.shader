...
{
    Properties { _Base("Base",2D)="white"{} _Color("Color", Color)=(1,1,1,1) }
    SubShader { Tags { "RenderPipeline"="UniversalPipeline" "RenderType"="Transparent" "Queue"="Transparent" } Blend SrcAlpha OneMinusSrcAlpha ZWrite Off
        Pass { Name "UniversalForward" Tags { "LightMode"="UniversalForward" } Cull Back
            ...
            CBUFFER_START(UnityPerMaterial) float4 _Base_ST; half4 _Color; CBUFFER_END
            ...
            half4 Frag(Varyings IN):SV_Target { half4 c=SAMPLE_TEXTURE2D(_Base,sampler_Base,IN.uv*_Base_ST.xy+_Base_ST.zw)*IN.color*_Color; return c; }
            ENDHLSL
        }
    }
}